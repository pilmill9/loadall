...
fdetnames=dir('c:\testloadall/detections/*.mat');

fnames = dir('c:\testloadall/localizations/*.mat');
...
if ~isempty(coord)    
    command=sprintf('import.py --file %s --sourcemap SIO.Loc.v570 --overwrite TRUE Localizations', filename);
    system(command);
end
